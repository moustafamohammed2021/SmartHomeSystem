...
# 1 "./MCAL_LAYER/ADC/hal_adc_config.h" 1
...
# 1 "./MCAL_LAYER/ADC/../interrupt/mcal_interrupt_gen_config.h" 1
# 13 "./MCAL_LAYER/ADC/../interrupt/mcal_interrupt_config.h" 2
# 57 "./MCAL_LAYER/ADC/../interrupt/mcal_interrupt_config.h"
typedef enum {
...
typedef struct {
    ...
    acquisition_time_t adc_time_conversion;
    adc_conversion_clock_select_t adc_clock_freq;
    uint8 volatge_reference :1;
    uint8 result_format :1;
    uint8 pins_reserved :6;
}adc_config_t;
...
# 33 "./application.h"
led_t Room1_Led={.port=PORTD_INDEX,.pin=GPIO_PIN0,.status=LED_OFF};
led_t Room2_Led={.port=PORTD_INDEX,.pin=GPIO_PIN1,.status=LED_OFF};
led_t Room3_Led={.port=PORTD_INDEX,.pin=GPIO_PIN2,.status=LED_OFF};
led_t Room4_Led={.port=PORTD_INDEX,.pin=GPIO_PIN3,.status=LED_OFF};
led_t TV_Led={.port=PORTD_INDEX,.pin=GPIO_PIN4,.status=LED_OFF};
led_t AIR_COND_Led={.port=PORTD_INDEX,.pin=GPIO_PIN5,.status=LED_OFF};


SPI_Config_t SPIObject={.SPI_InterruptHandler=SPI_InterrutHandler,.SPI_mode=SPI_SLAVE_MODE_SS_BIN_ENABLED,
                        .SPI_config.SPI_interrupt_enable=0x01,
...
adc_config_t ADC_Obj={.ADC_HandlerInterrupt=((void*)0),.adc_channel=ADC_CHANNEL_AN0,
                      .adc_clock_freq=ADC_CONVERSION_CLOCK_FOSC_DIV_4,
                      .adc_time_conversion=ADC_12_TAD,
                      .result_format=0x01U,
                      .volatge_reference=0x00U};
# 8 "application.c" 2
...
volatile uint16 u16RequiredTemperature=24;
volatile uint16 u16ReadingCounter=0;
volatile uint16 u16TempSensorReading=0;
volatile uint16 u16LastValueAirCondtioning=(uint8)0X00;

volatile uint8 u8Request=0XFF;
volatile uint8 u8Response=0XFF;
int main(void){
    ...
    led_initialize(&AIR_COND_Led);
    while(1){
        u8SPITranmitRecieve(u8Response);
        switch(u8Request){


            case 0X11:
                if((((PORTD >> 0) & (uint8)1))==1){
                    u8Response=0X01;
                }
                else if((((PORTD >> 0) & (uint8)1))==0){
                    u8Response=0X00;
                }
                else {}
                ...
                break;
            case 0X12:
                if((((PORTD >> 1) & (uint8)1))==1){
                    u8Response=0X01;
                }
                else if((((PORTD >> 1) & (uint8)1))==0){
                    u8Response=0X00;
                }
                else {}
                ...
                break;
            case 0X13:
                if((((PORTD >> 2) & (uint8)1))==1){
                    u8Response=0X01;
                }
                else if((((PORTD >> 2) & (uint8)1))==0){
                    u8Response=0X00;
                }
                else {}
                ...
                break;
            case 0X14:
                if((((PORTD >> 3) & (uint8)1))==1){
                    u8Response=0X01;
                }
                else if((((PORTD >> 3) & (uint8)1))==0){
                    u8Response=0X00;
                }
                else {}
                ...
                break;
            case 0X15:
                if((((PORTD >> 4) & (uint8)1))==1){
                    u8Response=0X01;
                }
                else if((((PORTD >> 4) & (uint8)1))==0){
                    u8Response=0X00;
                }
                else {}
                ...
                break;
            case 0X16:
                if((((PORTD >> 5) & (uint8)1))==1){
                    u8Response=0X01;
                }
                else if((((PORTD >> 5) & (uint8)1))==0){
                    u8Response=0X00;
                }
                else {}
            ...
            case 0X21:
                led_turn_on(&Room1_Led);
                break;
            case 0X22:
                led_turn_on(&Room2_Led);
                break;
            case 0X23:
                led_turn_on(&Room3_Led);
                break;
            case 0X24:
                led_turn_on(&Room4_Led);
                break;
            case 0X25:
                led_turn_on(&TV_Led);
                break;
            case 0X26:
                Timer1_Init(&Timer1Obj);
                led_turn_on(&AIR_COND_Led);
                break;

            case 0X31:
                led_turn_off(&Room1_Led);
                break;
            case 0X32:
                led_turn_off(&Room2_Led);
                break;
            case 0X33:
                led_turn_off(&Room3_Led);
                break;
            case 0X34:
                led_turn_off(&Room4_Led);
                break;
            case 0X35:
                led_turn_off(&TV_Led);
                break;
            case 0X36:
                ...
                led_turn_off(&AIR_COND_Led);
                break;
            case 0X40:
                u16RequiredTemperature=SSPBUF;
                break;
        }
        }
    return (0);
...
void Timer1Handler(void){
    u16ReadingCounter++;
    if(u16ReadingCounter >= 10){
        u16ReadingCounter=0;
        u16TempSensorReading=(0.5*GetResultFromTheADC());
        if(u16TempSensorReading >= (u16RequiredTemperature+1)){
            led_turn_on(&AIR_COND_Led);
            u16LastValueAirCondtioning=(uint8)0X01;
        }
        else if(u16TempSensorReading <= (u16RequiredTemperature-1)){
            led_turn_off(&AIR_COND_Led);
            u16LastValueAirCondtioning=(uint8)0X00 ;
        }
        else if(u16TempSensorReading == (u16RequiredTemperature+1)){
            if(u16LastValueAirCondtioning==(uint8)0X01){
                led_turn_on(&AIR_COND_Led);
            }
            else if(u16LastValueAirCondtioning==(uint8)0X00){
                led_turn_off(&AIR_COND_Led);
            }
        }
