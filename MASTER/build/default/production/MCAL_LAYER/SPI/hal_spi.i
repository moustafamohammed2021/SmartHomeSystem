...
# 11 "MCAL_LAYER/SPI/../GPIO/../mcal_std_types.h"
...
# 11 "MCAL_LAYER/SPI/../GPIO/../mcal_std_types.h" 2
...
# 1 "MCAL_LAYER/SPI/../GPIO/hal_gpio_config.h" 1
...
static void (*HandelerSPI)(void)=((void*)0);

static __attribute__((inline)) void SPI_Select_mode_and_pin_initialize(const SPI_Config_t *_spi_obj);
static __attribute__((inline)) void SPI_Select_the_clock_polarity(const SPI_Config_t *_spi_obj);
static __attribute__((inline)) void SPI_Select_the_clock_phase(const SPI_Config_t *_spi_obj);
static __attribute__((inline)) void SPI_Select_the_Sampling_receiving(const SPI_Config_t *_spi_obj);
static __attribute__((inline)) void SPI_configure_the_interrupt(const SPI_Config_t *_spi_obj);
...
Std_ReturnType SPI_Init(const SPI_Config_t *_spi_obj){
     Std_ReturnType ret=(Std_ReturnType)0x01;
        if (((void*)0)==_spi_obj){
            ...
            (SSPCON1bits.SSPM=_spi_obj->SPI_mode);
...
static __attribute__((inline)) void SPI_Select_the_clock_polarity(const SPI_Config_t *_spi_obj){
    if (0x01==_spi_obj->SPI_config.clock_polarity){
        (SSPCON1bits.CKP=1);
    }
    else if (0x00==_spi_obj->SPI_config.clock_polarity){
        (SSPCON1bits.CKP=0);
    }
}

static __attribute__((inline)) void SPI_Select_the_clock_phase(const SPI_Config_t *_spi_obj){
    if(0x01==_spi_obj->SPI_config.clock_phase){
        (SSPSTATbits.CKE=1);
    }
    else if(0x00==_spi_obj->SPI_config.clock_phase){
        (SSPSTATbits.CKE=0);
    }
}
static __attribute__((inline)) void SPI_Select_the_Sampling_receiving(const SPI_Config_t *_spi_obj){
    if (0x01==_spi_obj->SPI_config.sampling_receiving){
        (SSPSTATbits.SMP=1);
    }
    else if (0x00==_spi_obj->SPI_config.sampling_receiving){
        (SSPSTATbits.SMP=0);
    }
}
static __attribute__((inline)) void SPI_configure_the_interrupt(const SPI_Config_t *_spi_obj){
    if (0x01==_spi_obj->SPI_config.SPI_interrupt_enable){

       (PIE1bits.SSPIE=1);
       (PIR1bits.SSPIF=0);
# 180 "MCAL_LAYER/SPI/hal_spi.c"
            (INTCONbits.GIE=1);
            (INTCONbits.PEIE=1);
    ...
    else if (0x00==_spi_obj->SPI_config.SPI_interrupt_enable){
        (PIE1bits.SSPIE=0);
    }
...
uint8 u8SPITranmitRecieve(uint8 u8Data){
    uint8 LOC_DataReceived=0XFF;
    SPI_SendByteBlocking(u8Data);
    SPI_ReadByteBlocking(&LOC_DataReceived);
    return LOC_DataReceived;
}
