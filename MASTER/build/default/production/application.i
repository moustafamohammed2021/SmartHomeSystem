...
# 1 "./MCAL_LAYER/EEPROM/../interrupt/mcal_interrupt_gen_config.h" 1
# 13 "./MCAL_LAYER/EEPROM/../interrupt/mcal_interrupt_config.h" 2
# 57 "./MCAL_LAYER/EEPROM/../interrupt/mcal_interrupt_config.h"
typedef enum {
...
# 13 "./ECU_LAYER/char_lcd/ecu_char_lcd.h"
...
# 1 "./ECU_LAYER/key_pad/ecu_keypad.h" 1
# 11 "./ECU_LAYER/key_pad/ecu_keypad.h"
# 1 "./ECU_LAYER/key_pad/ecu_keypad_config.h" 1
# 11 "./ECU_LAYER/key_pad/ecu_keypad.h" 2
# 22 "./ECU_LAYER/key_pad/ecu_keypad.h"
typedef struct {
    pin_config_t pin_keypad_rows[4];
    pin_config_t pin_keypad_colums[4];
}keypad_t;
...
SPI_Config_t SPIObject={.SPI_InterruptHandler=((void*)0),.SPI_mode=SPI_MASTER_MODE_CLOCK_FOSC_DIV_4,
                        .SPI_config.SPI_interrupt_enable=0x00,
                        .SPI_config.clock_phase=0x00,
                        .SPI_config.clock_polarity=0x00,
                        .SPI_config.sampling_receiving=0x00};
...
volatile uint16 SessionCounter=0;
uint8 FlagTimeOut=(uint8)0;
int main(void) {

    led_initialize(&ADMIN_Led);
    led_initialize(&GUEST_Led);
    led_initialize(&BLOCK_Led);
    keypad_initialize(&Keypad_Obj);
    lcd_4bit_initialize(&Lcd_Obj);
    gpio_Pin_Intialize(&PIN_SelectChannel);
    SPI_Init(&SPIObject);
    ...
    uint8 u8KeyPadPressed=0XFF;
    uint8 u8LoginMode=(uint8)0;
    uint8 BlockModeFlag=(uint8)0;


    lcd_4bit_send_string_data(&Lcd_Obj,"Welcome to Smart");
    set_cursor_4bit_mode(&Lcd_Obj,2,3);
    lcd_4bit_send_string_data(&Lcd_Obj,"Home System");
    _TimerDelay_ms(2000);
    lcd_4bit_send_command(&Lcd_Obj,0x01);


    if(((uint8)0X00 !=u8PassIsSetOrNotSet((uint16)0X0000)) || ((uint8)0X00 !=u8PassIsSetOrNotSet((uint16)0X0001))){
        lcd_4bit_send_command(&Lcd_Obj,0x01);
        lcd_4bit_send_string_data(&Lcd_Obj,"Login For The");
        set_cursor_4bit_mode(&Lcd_Obj,2,3);
        lcd_4bit_send_string_data(&Lcd_Obj,"First Time");
        _TimerDelay_ms(2000);
        lcd_4bit_send_command(&Lcd_Obj,0x01);
        lcd_4bit_send_string_data(&Lcd_Obj,"Set Admin Pass ");
        set_cursor_4bit_mode(&Lcd_Obj,2,1);
        lcd_4bit_send_string_data(&Lcd_Obj,"Admin Pass:");

        uint8 PassCounter=0;
        uint8 Pass[(uint8)4]={0XFF,0XFF,0XFF,0XFF};
        while (PassCounter < (uint8)4){
            u8KeyPadPressed=0XFF;
            while(u8KeyPadPressed == 0XFF){
                keypad_get_value(&Keypad_Obj,&u8KeyPadPressed);
            }
            Pass[PassCounter]=u8KeyPadPressed;
            lcd_4bit_send_char_data(&Lcd_Obj,u8KeyPadPressed);
            _TimerDelay_ms((uint16)300);
            set_cursor_4bit_mode(&Lcd_Obj,2,12+PassCounter);
            lcd_4bit_send_char_data(&Lcd_Obj,(uint8)'*');
            _TimerDelay_ms(50);
            PassCounter++;
        }
        vWriteBlockOnEEPROM((uint16)0X0003,Pass,(uint8)4);
        eeprom_WriteDataByte((uint16)0X0000,(uint8)0X00);
        lcd_4bit_send_command(&Lcd_Obj,0x01);
        lcd_4bit_send_string_data(&Lcd_Obj,"Pass Saved");
        _TimerDelay_ms(1000);


        PassCounter=0;
        lcd_4bit_send_command(&Lcd_Obj,0x01);
        lcd_4bit_send_string_data(&Lcd_Obj,"Set Guest Pass ");
        set_cursor_4bit_mode(&Lcd_Obj,2,1);
        lcd_4bit_send_string_data(&Lcd_Obj,"Guest Pass:");
        while (PassCounter < (uint8)4){
            u8KeyPadPressed=0XFF;
            while(u8KeyPadPressed == 0XFF){
                keypad_get_value(&Keypad_Obj,&u8KeyPadPressed);
        ...
        vWriteBlockOnEEPROM((uint16)0X000A,Pass,(uint8)4);
        eeprom_WriteDataByte((uint16)0X0001,(uint8)0X00);
        lcd_4bit_send_command(&Lcd_Obj,0x01);
        lcd_4bit_send_string_data(&Lcd_Obj,"Pass Saved");
        _TimerDelay_ms(1000);
        lcd_4bit_send_command(&Lcd_Obj,0x01);
        eeprom_WriteDataByte((uint16)0X0002,(uint8)0);
    }
    else {
        BlockModeFlag=u8PassIsSetOrNotSet((uint16)0X0002);
    }
    while(1){
        u8KeyPadPressed=0XFF;
        uint8 PassTriesCount=0;
        if(FlagTimeOut==(uint8)1){
                Timer1_DeInit(&Timer1Obj);
                SessionCounter=0;
                u8LoginMode=(uint8)0;
                FlagTimeOut=(uint8)0;
                u8KeyPadPressed=0XFF;
                ...
                lcd_4bit_send_command(&Lcd_Obj,0x01);
                lcd_4bit_send_string_data(&Lcd_Obj,"Session Timeout");
                _TimerDelay_ms(1000);
            }
        while(u8LoginMode==(uint8)0){
            if(BlockModeFlag==(uint8)1){
                lcd_4bit_send_command(&Lcd_Obj,0x01);
                lcd_4bit_send_string_data(&Lcd_Obj,"Login Blocked");
                set_cursor_4bit_mode(&Lcd_Obj,2,1);
                lcd_4bit_send_string_data(&Lcd_Obj,"Wait 20 Seconds");
                led_turn_on(&BLOCK_Led);
                _TimerDelay_ms((uint32)20000);
                PassTriesCount=0;
                BlockModeFlag=(uint8)0;
                u8KeyPadPressed=0XFF;
                led_turn_off(&BLOCK_Led);
                eeprom_WriteDataByte((uint16)0X0002,(uint8)0);
            }
            lcd_4bit_send_command(&Lcd_Obj,0x01);
            lcd_4bit_send_string_data(&Lcd_Obj,"Select Mode :");
            set_cursor_4bit_mode(&Lcd_Obj,2,1);
            lcd_4bit_send_string_data(&Lcd_Obj,"0:Admin  1:Guest");
            u8KeyPadPressed=0XFF;
            while(u8KeyPadPressed == 0XFF){
                keypad_get_value(&Keypad_Obj,&u8KeyPadPressed);
            }
            if((u8KeyPadPressed != (uint8)'0') && (u8KeyPadPressed!=(uint8)'1') ){
                lcd_4bit_send_command(&Lcd_Obj,0x01);
                lcd_4bit_send_string_data(&Lcd_Obj,"Wrong Input");
                _TimerDelay_ms(1000);
                continue;
            }
            uint8 PassCounter=0;
            uint8 Pass[(uint8)4]={0XFF,0XFF,0XFF,0XFF};
            uint8 PassStored[(uint8)4]={0XFF,0XFF,0XFF,0XFF};

            switch(u8KeyPadPressed){
                case (uint8)'0':
                while(u8LoginMode != (uint8)1){
                    u8KeyPadPressed=0XFF;
                    lcd_4bit_send_command(&Lcd_Obj,0x01);
                    lcd_4bit_send_string_data(&Lcd_Obj,"Admin Mode");
                    set_cursor_4bit_mode(&Lcd_Obj,2,1);
                    lcd_4bit_send_string_data(&Lcd_Obj,"Admin Pass:");
                    _TimerDelay_ms(200);
                    while(PassCounter < (uint8)4){
                    ...
                    PassCounter=0;
                    vReadBlockOnEEPROM((uint16)0X0003,PassStored,(uint8)4);

                    if((uint8)1==u8ComparePass(PassStored,Pass,(uint8)4)){
                        u8LoginMode=(uint8)1;
                        PassTriesCount=0;
                        lcd_4bit_send_command(&Lcd_Obj,0x01);
                        lcd_4bit_send_string_data(&Lcd_Obj,"Correct Pass");
                        set_cursor_4bit_mode(&Lcd_Obj,2,1);
                        lcd_4bit_send_string_data(&Lcd_Obj,"Admin Mode");
                        _TimerDelay_ms(1000);
                    ...
                    else if ((uint8)0==u8ComparePass(PassStored,Pass,(uint8)4)){
                        PassTriesCount++;
                        u8LoginMode=(uint8)0;
                        lcd_4bit_send_command(&Lcd_Obj,0x01);
                        lcd_4bit_send_string_data(&Lcd_Obj,"Wrong Pass");
                        set_cursor_4bit_mode(&Lcd_Obj,2,1);
                        lcd_4bit_send_string_data(&Lcd_Obj,"Tries Left");
                        lcd_4bit_send_char_data(&Lcd_Obj,(uint8)3 -PassTriesCount+(uint8)'0');
                        _TimerDelay_ms(1000);
                        lcd_4bit_send_command(&Lcd_Obj,0x01);
                        if(PassTriesCount >= (uint8)3){
                            eeprom_WriteDataByte((uint16)0X0002,(uint8)1);
                            BlockModeFlag=(uint8)1;
                            break;
                        }
                    }
                 }
                ...
                case (uint8)'1':
                    while(u8LoginMode != (uint8)2){
                    u8KeyPadPressed=0XFF;
                    lcd_4bit_send_command(&Lcd_Obj,0x01);
                    lcd_4bit_send_string_data(&Lcd_Obj,"Guest Mode");
                    set_cursor_4bit_mode(&Lcd_Obj,2,1);
                    lcd_4bit_send_string_data(&Lcd_Obj,"Guest Pass:");
                    _TimerDelay_ms(200);
                    while(PassCounter <(uint8)4){
                    ...
                    PassCounter=0;
                    vReadBlockOnEEPROM((uint16)0X000A,PassStored,(uint8)4);
        ...
        uint8 ShowMenu=(uint8)1;

        while(FlagTimeOut != (uint8)1){
            u8KeyPadPressed=0XFF;
            switch(ShowMenu){
                case (uint8)1:
                    do{
                        ...
                        lcd_4bit_send_command(&Lcd_Obj,0x01);
                        lcd_4bit_send_string_data(&Lcd_Obj,"1:Room1 2:Room2");
                        set_cursor_4bit_mode(&Lcd_Obj,2,1);
                        if(u8LoginMode==(uint8)1){
                            lcd_4bit_send_string_data(&Lcd_Obj,"3:Room3 4:More");
                        }
                        else if (u8LoginMode==(uint8)2){
                            lcd_4bit_send_string_data(&Lcd_Obj,"3:Room3 4:Room4");
                        }
                        u8KeyPadPressed=u8GetPressedValue(u8LoginMode);
                        _TimerDelay_ms(100);
                        if(u8KeyPadPressed==(uint8)'1'){
                            ShowMenu=(uint8)3;
                        }
                        else if(u8KeyPadPressed==(uint8)'2'){
                            ShowMenu=(uint8)4;
                        }
                        else if(u8KeyPadPressed==(uint8)'3'){
                            ShowMenu=(uint8)5;
                        }
                        else if((u8KeyPadPressed==(uint8)'4') && (u8LoginMode==(uint8)1)){
                            ShowMenu=(uint8)2;
                        }
                        else if((u8KeyPadPressed==(uint8)'4') && (u8LoginMode==(uint8)2)){
                             ShowMenu=(uint8)6;
                        }
                        else if(u8KeyPadPressed != 0XFF){
                            lcd_4bit_send_command(&Lcd_Obj,0x01);
                            lcd_4bit_send_string_data(&Lcd_Obj,"Wrong Input");
                            _TimerDelay_ms(500);
                        }
                    }while(((u8KeyPadPressed < '1') || (u8KeyPadPressed > '4')) && (FlagTimeOut == (uint8)0));
                    break;
                case (uint8)2:
                    do{
                    lcd_4bit_send_command(&Lcd_Obj,0x01);
                    lcd_4bit_send_string_data(&Lcd_Obj,"1:Room4 2:TV");
                    set_cursor_4bit_mode(&Lcd_Obj,2,1);
                    lcd_4bit_send_string_data(&Lcd_Obj,"3:AirCond 4:RET");
                    u8KeyPadPressed=u8GetPressedValue(u8LoginMode);
                    _TimerDelay_ms(100);
                    if(u8KeyPadPressed==(uint8)'1'){
                            ShowMenu=(uint8)6;
                        ...
                        else if(u8KeyPadPressed==(uint8)'4'){
                            ShowMenu=(uint8)1;
                        }
                        else if(u8KeyPadPressed != 0XFF){
                            lcd_4bit_send_command(&Lcd_Obj,0x01);
                ...
                case (uint8)8:
                    do{
                    lcd_4bit_send_command(&Lcd_Obj,0x01);
                    lcd_4bit_send_string_data(&Lcd_Obj,"1:Set Temperature");
                    set_cursor_4bit_mode(&Lcd_Obj,2,1);
                    lcd_4bit_send_string_data(&Lcd_Obj,"2:Control 0:RET");
                    u8KeyPadPressed=u8GetPressedValue(u8LoginMode);
                    _TimerDelay_ms(100);
                    if(u8KeyPadPressed==(uint8)'1'){
                            ShowMenu=(uint8)10;
                        ...
                        else if(u8KeyPadPressed==(uint8)'0'){
                            ShowMenu=(uint8)2;
                        }
                        else if(u8KeyPadPressed != 0XFF){
                            lcd_4bit_send_command(&Lcd_Obj,0x01);
                    ...
                    }while(((u8KeyPadPressed < '0') || (u8KeyPadPressed > '2')) && (FlagTimeOut == (uint8)0));
                    break;
                case (uint8)3:
                    vMenuOption((uint8)3,u8LoginMode);
                    ShowMenu=(uint8)1;
                    break;
                case (uint8)4:
                    vMenuOption((uint8)4,u8LoginMode);
                    ShowMenu=(uint8)1;
                    break;
                case (uint8)5:
                    vMenuOption((uint8)5,u8LoginMode);
                    ShowMenu=(uint8)1;
                    break;
                case (uint8)6:
                    vMenuOption((uint8)6,u8LoginMode);
                    if(u8LoginMode==(uint8)2){
                        ShowMenu=(uint8)1;
                    }
                    else if(u8LoginMode==(uint8)1){
                    ShowMenu=(uint8)2;
                    }
                    break;
                case (uint8)7:
                    vMenuOption((uint8)7,u8LoginMode);
                    ShowMenu=(uint8)2;
                    break;
                case (uint8)9:
                    vMenuOption((uint8)9,u8LoginMode);
                    ShowMenu=(uint8)8;
                    break;
                case (uint8)10:
                    Temperature=0;
                    while(Temperature==0 && FlagTimeOut==(uint8)0){
                        lcd_4bit_send_command(&Lcd_Obj,0x01);
                        lcd_4bit_send_string_data(&Lcd_Obj,"Set Temp.:__");
                        lcd_4bit_send_char_data(&Lcd_Obj,(uint8)0XDF);
                        lcd_4bit_send_char_data(&Lcd_Obj,'C');
                        set_cursor_4bit_mode(&Lcd_Obj,1,11);
                        _TimerDelay_ms(200);
                        ...
                        if(u8KeyPadPressed < '0' || u8KeyPadPressed > '9'){
                            lcd_4bit_send_command(&Lcd_Obj,0x01);
                            lcd_4bit_send_string_data(&Lcd_Obj,"Wrong Input");
                            _TimerDelay_ms(500);
                            continue;
                            ...
                            lcd_4bit_send_char_data(&Lcd_Obj,u8KeyPadPressed);
                            TempTens=u8KeyPadPressed-(uint8)'0';
                            u8KeyPadPressed=0XFF;
                        }
                        ...
                        if(u8KeyPadPressed < '0' || u8KeyPadPressed > '9'){
                            ...
                            lcd_4bit_send_char_data(&Lcd_Obj,u8KeyPadPressed);
                            TempOnes=u8KeyPadPressed-(uint8)'0';
                            u8KeyPadPressed=0XFF;
                        }
                        Temperature=TempTens*10+TempOnes;
                        u8SpiTranmitRecieve(0X40);
                        _TimerDelay_ms(200);
                        u8SpiTranmitRecieve(Temperature);
                        _TimerDelay_ms(200);
                         lcd_4bit_send_command(&Lcd_Obj,0x01);
                         lcd_4bit_send_string_data(&Lcd_Obj,"Temperature Sent");
                         _TimerDelay_ms(500);
                    }
                    ShowMenu=(uint8)8;
                    break;
