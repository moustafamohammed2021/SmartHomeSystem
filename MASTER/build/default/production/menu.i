...
# 1 "./ECU_LAYER/char_lcd/../../MCAL_Layer/GPIO/hal_gpio_config.h" 1
# 14 "./ECU_LAYER/char_lcd/../../MCAL_Layer/GPIO/hal_gpio.h" 2
# 32 "./ECU_LAYER/char_lcd/../../MCAL_Layer/GPIO/hal_gpio.h"
typedef enum{
    GPIO_LOW=0,
    GPIO_HIGH
}logic_t;
...
# 11 "./MCAL_LAYER/SPI/../interrupt/mcal_internal_interrupt.h"
# 1 "./MCAL_LAYER/SPI/../interrupt/mcal_interrupt_config.h" 1
# 13 "./MCAL_LAYER/SPI/../interrupt/mcal_interrupt_config.h"
# 1 "./MCAL_LAYER/SPI/../interrupt/mcal_interrupt_gen_config.h" 1
# 13 "./MCAL_LAYER/SPI/../interrupt/mcal_interrupt_config.h" 2
# 57 "./MCAL_LAYER/SPI/../interrupt/mcal_interrupt_config.h"
typedef enum {
    INTERRUPT_LOW_PRIORITY=0,
    INTERRUPT_HIGH_PRIORITY
}interrupt_priority_t;
# 11 "./MCAL_LAYER/SPI/../interrupt/mcal_internal_interrupt.h" 2
...
extern volatile uint16 SessionCounter;
extern uint8 FlagTimeOut;

uint8 u8ComparePass(const uint8 *Pass1,const uint8 *Pass2,uint8 Size){
    uint8 PassCounter=0;
    uint8 PassFlag=(uint8)1;
...
void vMenuOption(const uint8 u8SelectedRoom,const uint8 u8LonginMode){
    ...
    uint8 u8Response=0XFF;
    uint8 u8keyPressed=0XFF;
    do{
     ...
     switch(u8SelectedRoom){
        ...
        case (uint8)9:
          ...
          u8SpiTranmitRecieve(u8StatusCode);
          _TimerDelay_ms(100);
          u8Response=u8SpiTranmitRecieve(u8StatusCode);
          if(u8Response==0X01){
              lcd_4bit_send_string_data(&Lcd_Obj,"ON");
          }
          else if (u8Response==0X00){
              lcd_4bit_send_string_data(&Lcd_Obj,"OFF");
          }
          else { }
          set_cursor_4bit_mode(&Lcd_Obj,2,1);
          lcd_4bit_send_string_data(&Lcd_Obj,"1-ON 2-OFF 0-RET");
          u8keyPressed=u8GetPressedValue(u8LonginMode);
          _TimerDelay_ms(100);
          if(u8keyPressed=='1'){
              u8SpiTranmitRecieve(u8TurnOnCode);
          }
          else if(u8keyPressed=='2'){
              u8SpiTranmitRecieve(u8TurnoffCode);
          }
          else if ((u8keyPressed != 0XFF) && (u8keyPressed != '0')){
              lcd_4bit_send_command(&Lcd_Obj,0x01);
              lcd_4bit_send_string_data(&Lcd_Obj,"Wrong Input");
              _TimerDelay_ms(500);
          }
    }while(((u8keyPressed < '0') || (u8keyPressed > '2')) && (FlagTimeOut == (uint8)0));
}

uint8 u8GetPressedValue(const uint8 u8LoginMode){
    uint8 u8LockeyPressed=0XFF;
    while (u8LockeyPressed == 0XFF){
        if((SessionCounter >= (uint16)3000 && u8LoginMode==(uint8)1) || (SessionCounter >= (uint16)2000 && u8LoginMode==(uint8)2)){
            FlagTimeOut=(uint8)1;
            break;
        }
        keypad_get_value(&Keypad_Obj,&u8LockeyPressed);
    }
    return u8LockeyPressed;
}
